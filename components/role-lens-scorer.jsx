...
export default function RoleLensScorer() {
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `## ROLE LENS (Company Side)\n\n${roleLens.trim()}\n\n---\n\n## CANDIDATE PROFILE\n\n${candidateInput.trim()}\n\n---\n\nScore this candidate against the role lens. Return ONLY valid JSON.`
          }],
        }),
      });
      if (!response.ok) {
      ...
      if (!text) throw new Error("Empty response — model returned no text content");
      if (data.stop_reason === "max_tokens") throw new Error("Response truncated — max_tokens hit. Try Quick Screen depth.");
      const clean = text.replace(/```json|```/g, "").trim();
      try {
        setResult(JSON.parse(clean));
      } catch (parseErr) {
        throw new Error("JSON parse failed. Raw response: " + clean.substring(0, 200));
      }
    } catch (err) {
  ...
  const reset = () => { setResult(null); setError(null); setCandidateInput(""); setTimeout(() => candidateRef.current?.focus(), 50); };
